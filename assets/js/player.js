window.addEventListener('message', e => {
  ...
  const langs = { "pt-BR": "Português (BR)", "en-US": "English (US)", "en-GB": "English (UK)", "es-419": "Español (LA)", "es-ES": "Español (ES)", "pt-PT": "Português (PT)", "fr-FR": "Français (FR)", "de-DE": "Deutsch (DE)", "ar-ME": "(ME) عربي", "it-IT": "Italiano (IT)", "ru-RU": "Русский (RU)", 'off': 'off' };
  ...
  for (const stream of streamList) {
    ...
    if (streamLang == sourceLocale) {
      sources.push({ 'type': 'm3u8', 'file': stream.url });
    };
  }
  ...
  function getTracks() {
    return streamList.map(({ url, hardsub_locale }) => {
  ...
  function getSourceLocale(userLang) {
    const locale = streamList.find(({ hardsub_locale }) => hardsub_locale == userLang);
